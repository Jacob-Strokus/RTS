...
public class ReplayWindow : EditorWindow
{
    ...
    [System.Serializable]
    private class ReplaySerializable{ public List<FrontierAges.Sim.DamageEvent> damage; public List<Simulator.SimEvent> events_; public List<Simulator.ReplayTimeline.TickSummary> ticks; public List<FrontierAges.Sim.Snapshot> snaps; public int lastTick; public int snapInterval;
        public ReplaySerializable(){}
        public ReplaySerializable(Simulator.ReplayTimeline rep){ damage = new List<FrontierAges.Sim.DamageEvent>(rep.DamageEvents); events_ = new List<Simulator.SimEvent>(rep.SimEvents); ticks = new List<Simulator.ReplayTimeline.TickSummary>(rep.Ticks); snaps = new List<FrontierAges.Sim.Snapshot>(rep.Snapshots); lastTick = rep.LastRecordedTick; snapInterval = rep.SnapshotInterval; }
        public Simulator.ReplayTimeline ToRuntime(){ var r = new Simulator.ReplayTimeline(); r.SnapshotInterval = snapInterval>0?snapInterval:50; r.Begin(0); r.DamageEvents.AddRange(damage??new List<FrontierAges.Sim.DamageEvent>()); r.SimEvents.AddRange(events_??new List<Simulator.SimEvent>()); r.Ticks.AddRange(ticks??new List<Simulator.ReplayTimeline.TickSummary>()); r.Snapshots.AddRange(snaps??new List<FrontierAges.Sim.Snapshot>()); r.LastRecordedTick = lastTick; return r; }
    }
